...
resource "aws_s3_bucket_ownership_controls" "codepipeline_bucket_s3_bucket_acl_ownership" {
...
resource "aws_codepipeline" "codepipeline" {
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ConnectionArn   = aws_codestarconnections_connection.incedo_connection.arn
...
resource "aws_codestarconnections_connection" "incedo_connection" {
  name          = "Incedo-connection"
